...
- **Timed stop (GUI)**: Settings → Access defaults to stopping after 2 hours, with Never, 10 minutes, 1/2/6/24 hours, and a custom hours/minutes duration (1 minute–99 hours 59 minutes). The preference persists; each successful start, file replacement, or timer-setting change starts a fresh countdown. Text edits and network/port rebinding retain the deadline. The timer uses wall time, runs with the window closed, and checks overdue deadlines on wake. Expiry calls `stop()`, stopping file/text/receive service and rotating credentials while retaining the selected content. Headless mode is unchanged.
