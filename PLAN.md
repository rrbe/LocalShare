...
> 3. **Markdown 链接/图片协议白名单**：`.md` 走 marked 预览渲染、不经上面的去势，故单独在 `MarkdownViewer.rendererConfig` 覆盖 marked 的 `link`/`image` 渲染器——丢弃 `javascript:`/`vbscript:`/`data:`（链接）这类点开即同源执行脚本的协议（判协议前先剥码点 ≤32 的字符，挡 `java<TAB>script:` 绕过），堵「恶意 `.md` 里一个 `[x](javascript:…)` 链接被点开即跑脚本」；安全 URL 返回 `false` 走 marked 默认渲染，不误伤正常链接，`data:image` 内联图放行。配置用 `/* MD-RENDERER-CONFIG */` 标记，`tools/smoke-md-link-sanitize.cjs` 连同**真实 vendored marked** 一起在 node 里跑断言（测真配置非复刻）。
>
