#!/usr/bin/env bash
# 防回归冒烟测：收文本（手机→Mac，v2）。覆盖只收模式、闸门关、以及与文件夹分享共存三形态：
#  · 只收模式 URL 直指 /ls/send；GET /ls/send 出发送表单（textarea + 发送钮），首访 302 清洗 token；
#  · POST /ls/text 收原文（text/plain），LOG 回读逐字一致（含 UTF-8 与 < 不被破坏）；
#  · 无 token → 403；空白 → 400；超 64KB → 413；
#  · 闸门关（仅文件夹）时 /ls/send 404、POST /ls/text 403、列表页不出发送表单；
...
wait_up "$PORT" "/ls/send"
base="http://127.0.0.1:$PORT"

echo "── 只收模式：headless URL 直指 /ls/send"
grep -q "LS_URL .*/ls/send?t=$TOK" /tmp/ls_recv_url.$$ && ok "URL 指向 /ls/send" || bad "URL 未指向 /ls/send：$(cat /tmp/ls_recv_url.$$)"

echo "── 导航（Accept text/html）首访 302 清洗 token"
S=$(curl -s -o /dev/null -w '%{http_code}' -H 'Accept: text/html' "$base/ls/send?t=$TOK")
[ "$S" = "302" ] && ok "302 清洗" || bad "应 302 实为 $S"

echo "── GET /ls/send（cookie）出发送表单"
P=$(curl -s -H 'Accept: text/html' -b "ls_token=$TOK" "$base/ls/send")
...
# ── 2. 闸门关（仅文件夹分享）：/ls/send 404、POST 403、列表无表单 ──
...
echo "── 闸门关：/ls/send 404"
S=$(curl -s -o /dev/null -w '%{http_code}' -b "ls_token=$TOK" "$base/ls/send")
[ "$S" = "404" ] && ok "/ls/send 404" || bad "应 404 实为 $S"
