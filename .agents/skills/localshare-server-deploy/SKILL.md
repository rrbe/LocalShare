...
Server 默认使用 `/var/lib/localshare`；只有用户明确选择其他目录时才追加 `--state-dir`，并同步调整 `ReadWritePaths`。
...
```text
/usr/local/bin/localshare-server serve --listen :<port> --public-url <public-base-url>
```
